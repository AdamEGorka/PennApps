...
class GraphVisualizer:
    ...
    def check_collision(self, mouse_pos, node):
        x, y = mouse_pos
        dist = ((node.x - x) ** 2 + (node.y - y) ** 2) ** 0.5
        return dist <= node.radius

    def check_node_collision(self, node1, node2):
    ...
    def add_connection(self, n1, n2):
        self.add_event(lambda: self.__add_connection(n1, n2))
    ...
    def remove_connection(self, n1, n2):
        self.add_event(lambda: self.__remove_connection(n1, n2))

    def __remove_connection(self, n1, n2):
        n1.connections.remove(n2)
        n2.connections.remove(n1)

        
    def add_node(self, x = None, y = None, color = BLUE, id = None):
        self.add_event(lambda: self.__add_node(x, y, color, id))
        
    def __add_node(self, x, y, color = BLUE, id = None):
        print("adding node")
        radius = 30
        global highest_id
        global nodes    
        ...
        nodes[new_id] = self.Node(new_id, x, y, radius, color)
        
    def remove_node(self, id):
        self.add_event(lambda: self.__remove_node(id))
        
    def __remove_node(self, id):
        global nodes
        n = nodes[id]
    ...
    def add_event(self, event_function):
    ...
    def visualizer(self):
        ...
        step_button = self.Button(600, 500, 150, 50, BLUE, "Step")
        ...
        while True:
            ...
            for i in range(len(nodes)):
                for j in range(i + 1, len(nodes)):
                    if self.check_node_collision(nodes[i], nodes[j]):
